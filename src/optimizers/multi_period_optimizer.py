...
try:
    from .market_signals import SyntheticSignals, GARCHSignals
except ImportError:
    from market_signals import SyntheticSignals, GARCHSignals
...
class EfficientFrontier:
    def __init__(self):
        self._frontier_results = None

    def calculate_efficient_frontier(self, 
                                     risk_aversion_levels: np.ndarray, 
                                     rebalance_problem: dict) -> None:
        result_container = []
        ...
        self._frontier_results = pd.concat(result_container)
        self.output_excel_report()

    def output_excel_report(self, output_file: str | None = None) -> str:
        results = self.efficient_frontier_results
        output_path = Path(output_file or f"optimizer_results/efficient_frontier_results_{pd.Timestamp.now():%Y-%m-%d}.xlsx")
    ...
    def plot_efficient_frontier(self, risk_aversion_levels: np.ndarray) -> None:
        results = self.efficient_frontier_results
    ...
    def plot_portfolio_composition(self) -> None:
        results = self.efficient_frontier_results.copy()
    ...
    @property
    def efficient_frontier_results(self):
        if self._frontier_results is None:
            raise ValueError("calculate_efficient_frontier must be called before extracting results")
        return self._frontier_results
